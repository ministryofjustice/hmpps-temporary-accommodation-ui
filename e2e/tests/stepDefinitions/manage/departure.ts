...
import { cas3BookingFactory, departureFactory, newDepartureFactory } from '../../../../server/testutils/factories'
...
Given('I mark the booking as departed', () => {
  cy.then(function _() {
    ...
    const bookingDeparturePage = Page.verifyOnPage(BookingDepartureNewPage, this.premises, this.bedspace, this.booking)
...
Given('I edit the departed booking', () => {
  cy.then(function _() {
    ...
    const bookingDeparturePage = Page.verifyOnPage(BookingDepartureEditPage, this.premises, this.bedspace, this.booking)
    bookingDeparturePage.shouldShowBookingDetails()
    bookingDeparturePage.completeForm(newDeparture)
...
Then('I should see a list of the problems encountered marking the booking as departed', () => {
  cy.then(function _() {
    const page = Page.verifyOnPage(BookingDepartureNewPage, this.premises, this.bedspace, this.booking)
...
Then('I should see a list of the problems encountered editing the departed booking', () => {
  cy.then(function _() {
    const page = Page.verifyOnPage(BookingDepartureEditPage, this.premises, this.bedspace, this.booking)
    page.shouldShowErrorMessagesForFields(['dateTime', 'reasonId', 'moveOnCategoryId'])
