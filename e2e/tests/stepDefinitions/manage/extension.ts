...
import { cas3BookingFactory, cas3ExtensionFactory, newExtensionFactory } from '../../../../server/testutils/factories'
  ...
  cy.then(function _() {
    ...
    const newExtension = newExtensionFactory.build({
      newDepartureDate: DateFormats.dateObjToIsoDate(
        faker.date.soon({ days: 83, refDate: DateFormats.isoToDateObj(this.booking.arrivalDate) }),
      ),
    })
    ...
    const extendedBooking = cas3BookingFactory.build({
      ...this.booking,
...
Then('I should see a list of the problems encountered extending the booking', () => {
  cy.then(function _() {
    const page = Page.verifyOnPage(BookingExtensionNewPage, this.premises, this.bedspace, this.booking)
