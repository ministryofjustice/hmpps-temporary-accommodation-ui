...
context('Booking turnarounds', () => {
  ...
  it('navigates to the change turnaround page', () => {
    ...
    Page.verifyOnPage(BookingTurnaroundEditPage, premises, room, booking)
  })
  ...
  it('allows me to change a turnaround', () => {
    ...
    const page = BookingTurnaroundEditPage.visit(premises, room, booking)
    ...
    cy.task('stubTurnaroundUpdate', { premisesId: premises.id, bookingId: booking.id, turnaround })

    page.completeForm(updateTurnaround)

    // Then the turnaround should have been created in the API
    cy.task('verifyTurnaroundUpdate', { premisesId: premises.id, bookingId: booking.id }).then(requests => {
      expect(requests).to.have.length(1)
      const requestBody = JSON.parse(requests[0].body)
      expect(requestBody.workingDays).equal(updateTurnaround.workingDays)
    })
  ...
  it('shows errors when the API returns an error', () => {
    ...
    cy.task('stubTurnaroundUpdateErrors', {
  ...
  it('shows errors when the API returns a 409 conflict error', () => {
    ...
    cy.task('stubTurnaroundUpdateConflictError', {
      premisesId: premises.id,
      bookingId: booking.id,
