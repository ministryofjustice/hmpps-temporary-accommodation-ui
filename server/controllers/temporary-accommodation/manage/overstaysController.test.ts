...
describe('OverstaysController', () => {
  ...
  describe('create', () => {
    ...
    it('redirects to the departure page with errors if the departure is in the future', async () => {
      // newDepartureDate is 84 days in the future
      const newOverstay = cas3NewOverstayFactory.build()
      const newDeparture = cas3NewDepartureFactory.build({
        dateTime: DateFormats.dateObjToIsoDateTime(DateFormats.isoToDateObj(newOverstay.newDepartureDate)),
      })
      ...
      expect(insertGenericError).toHaveBeenCalledWith(expect.any(Error), 'dateTime', 'departureDateInFuture')
