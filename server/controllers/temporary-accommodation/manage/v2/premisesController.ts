...
import { PremisesSearchParameters, PremisesShowTabs } from '@approved-premises/ui'
...
export default class PremisesController {
  ...
  show(activeTab: PremisesShowTabs): RequestHandler {
    return async (req: Request, res: Response) => {
      const callConfig = extractCallConfig(req)
      ...
      const bedspaces = await this.bedspaceService.getBedspacesForPremises(callConfig, premisesId)
      ...
      return res.render('temporary-accommodation/v2/premises/show', {
        ...
        bedspaceSummaries,
        actions: [],
        showPremises: activeTab === 'premises',
        subNavArr: showPropertySubNavArray(premisesId, activeTab),
      })
    }
  }
