...
export default function routes(controllers: Controllers, services: Services, router: Router): Router {
  ...
  if (!config.flags.turnaroundsDisabled) {
    get(paths.bookings.turnarounds.edit.pattern, turnaroundsController.edit(), {
      auditEvent: 'VIEW_BOOKING_EDIT_TURNAROUND',
    })
    put(paths.bookings.turnarounds.update.pattern, turnaroundsController.update(), {
      redirectAuditEventSpecs: [
        {
          path: paths.bookings.turnarounds.edit.pattern,
          auditEvent: 'EDIT_BOOKING_TURNAROUND_FAILURE',
        },
        {
          path: paths.bookings.show.pattern,
          auditEvent: 'EDIT_BOOKING_TURNAROUND_SUCCESS',
        },
      ],
    })
  }
