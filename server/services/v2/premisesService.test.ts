...
describe('PremisesService', () => {
  ...
  describe('tableRows', () => {
    ...
    it.each([
      [
        [premisesSummary1, premisesSummary2, premisesSummary3],
        undefined,
        [premisesSummary2, premisesSummary1, premisesSummary3],
      ],
      [[premisesSummary1, premisesSummary2], 'Westminster', [premisesSummary2, premisesSummary1]],
      [[], 'XYZ', []],
    ])(
      'returns a sorted table view of the premises for Temporary Accommodation with an optional search for an address',
      async (premises, postcodeOrAddress, expectedPremises) => {
        ...
        const expectedRows = expectedPremises.map(prem => {
          ...
          return [
            ...
            { html: bedspaces },
            { text: prem.pdu },
            { html: `<a href="/path/with/place/context">Manage</a>` },
          ]
        })
        ...
        expectedPremises.forEach(prem => {
          expect(addPlaceContext).toHaveBeenCalledWith(paths.premises.v2.show({ premisesId: prem.id }), placeContext)
        })
      },
    )
  })
